...
public class Export {
    ...
    private static final int[] QUESTION_FIELDS = new int[] { Question.ID, Question.NAME, Question.TYPE, Question.LIST_TYPE };
    ...
    private static final Function<String,String> TEMPLATE_QUAL = (catalogName) ->
            format("'%s'=\"%s\"", Template.CATALOG, catalogName);
    private static final BiFunction<String,String,String> TEMPLATE_NAME_QUAL = (catalog, template) ->
    ...
    public static void export(Config config, ARServerUser user, Template template) {
        ...
        // sanitize the template name and setup the output directory for the current template
        ...
        try {
            ...
            // create the queues of submissions, at each step the submissions get decorated with
            ...
            try (FileWriter fileWriter = new FileWriter(new File(templateDir, "error.txt"))) {
                ...
                for (StackTraceElement ste : e.getStackTrace()) fileWriter.write(ste.toString());
    ...
    public static List<String> getCatalogNames(ARServerUser user, int queryLimit) {
        return ArsHelper.getAllEntries(user, Catalog.FORM, CATALOG_QUAL, queryLimit, CATALOG_ORDER,
                CATALOG_FIELDS).stream().map(Catalog::new).map(Catalog::getName).collect(Collectors.toList());
    }

    public static List<Template> getTemplates(ARServerUser user, int queryLimit, String catalogName) {
        return ArsHelper.getAllEntries(user, Template.FORM, TEMPLATE_QUAL.apply(catalogName),
                queryLimit, TEMPLATE_ORDER, TEMPLATE_FIELDS)
                .stream().map(Template::new).collect(Collectors.toList());
    }

    public static Template getTemplate(ARServerUser user, String catalogName, String templateName) {
        Entry entry = ArsHelper.getOneEntry(user, Template.FORM,
                TEMPLATE_NAME_QUAL.apply(catalogName, templateName), TEMPLATE_FIELDS);
    ...
    public static List<Question> getQuestions(ARServerUser user, int queryLimit, String templateId) {
        return ArsHelper.getAllEntries(user, Question.FORM, QUESTION_QUAL.apply(templateId),
                queryLimit, QUESTION_ORDER, QUESTION_FIELDS)
                .stream().map(Question::new).collect(Collectors.toList());
    }
